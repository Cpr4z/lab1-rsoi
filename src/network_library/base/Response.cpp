...
network::http_code_t BeastResp::GetStatus() const
{
    if (!m_isFromResp)
        return m_status;

    return static_cast<network::http_code_t>(m_resp.result_int());
}
...
void BeastResp::SetStatus(network::http_code_t status)
{
    m_status = status;
}
